...
BEGIN_MESSAGE_MAP(CImageProcessing1Doc, CDocument)
	...
	ON_COMMAND(ID_HISTO_EQUAL, &CImageProcessing1Doc::OnHistoEqual)
	...
	ON_COMMAND(ID_TRANSLATION, &CImageProcessing1Doc::OnTranslation)
	ON_COMMAND(ID_MIRROR_HOR, &CImageProcessing1Doc::OnMirrorHor)
	ON_COMMAND(ID_MIRROR_VER, &CImageProcessing1Doc::OnMirrorVer)
	ON_COMMAND(ID_ROTATION, &CImageProcessing1Doc::OnRotation)
	ON_COMMAND(ID_FRAME_SUM, &CImageProcessing1Doc::OnFrameSum)
	ON_COMMAND(ID_FRAME_SUB, &CImageProcessing1Doc::OnFrameSub)
	ON_COMMAND(ID_FRAME_MUL, &CImageProcessing1Doc::OnFrameMul)
	ON_COMMAND(ID_FRAME_DIV, &CImageProcessing1Doc::OnFrameDiv)
	ON_COMMAND(ID_FRAME_AND, &CImageProcessing1Doc::OnFrameAnd)
	ON_COMMAND(ID_FRAME_OR, &CImageProcessing1Doc::OnFrameOr)
	ON_COMMAND(ID_FRAME_COMB, &CImageProcessing1Doc::OnFrameComb)
	ON_COMMAND(ID_BINARY_EROSION, &CImageProcessing1Doc::OnBinaryErosion)
	ON_COMMAND(ID_BINARY_DILATION, &CImageProcessing1Doc::OnBinaryDilation)
	ON_COMMAND(ID_GRAY_EROSION, &CImageProcessing1Doc::OnGrayErosion)
	ON_COMMAND(ID_GRAY_DILATION, &CImageProcessing1Doc::OnGrayDilation)
	ON_COMMAND(ID_LOW_PASS_FILTER, &CImageProcessing1Doc::OnLowPassFilter)
	ON_COMMAND(ID_HIGH_PASS_FILTER, &CImageProcessing1Doc::OnHighPassFilter)
	ON_COMMAND(ID_MEAN_FILTER, &CImageProcessing1Doc::OnMeanFilter)
	ON_COMMAND(ID_MEDIAN_FILTER, &CImageProcessing1Doc::OnMedianFilter)
	ON_COMMAND(ID_MAX_FILTER, &CImageProcessing1Doc::OnMaxFilter)
	ON_COMMAND(ID_MIN_FILTER, &CImageProcessing1Doc::OnMinFilter)
	ON_COMMAND(ID_FFT_2D, &CImageProcessing1Doc::OnFft2d)
	ON_COMMAND(ID_IFFT_2D, &CImageProcessing1Doc::OnIfft2d)
	ON_COMMAND(ID_LPF_FREQUENCY, &CImageProcessing1Doc::OnLpfFrequency)
	ON_COMMAND(ID_HPF_FREQUENCY, &CImageProcessing1Doc::OnHpfFrequency)
END_MESSAGE_MAP()
...
void CImageProcessing1Doc::OnUpSampling()
{
	...
	if (dlg.DoModal() == IDOK) { // DoModal 대화상자의 활성화 여부
		...
		*/
		...
		for (i = 0; i < m_height; i++)
			for (j = 0; j < m_width; j++)
				for (m = 0; m < dlg.m_UpSampleRate; m++)
					for (n = 0; n < dlg.m_UpSampleRate; n++)
						tempImage[i + n][j + m] = m_InputImage[i * m_width + j];
	//copy
		for (i = 0; i < m_Re_height; i++)
			for (j = 0; j < m_Re_width; j++)
				m_OutputImage[i + j] = tempImage[i][j];
	//output
	*/
		/*
		for (i = 0; i < m_height; i++) {
			for (j = 0; j < m_width; j++) {
				tempImage[i][j] = m_InputImage[i * m_width + j];
			}//inputimage
		}

		m_OutputImage[i * dlg.m_UpSampleRate * m_Re_width + dlg.m_UpSampleRate * j] = tempImage[i][j];

		for (i = 0; i < dlg.m_UpSampleRate * m_Re_width; i++) {
			for (j = 0; j < dlg.m_UpSampleRate * m_width; j++) {
				tempImage[i][j] = m_InputImage[i * m_width + j];
			...
			for (j = 0; j < dlg.m_UpSampleRate * m_width; j++) {
				...
				for (m = 0; m < 3; m++) {
					for (n = 0; n < 3; n++) {
						...
						tempImage[i+m][j+n] = wf[m][n] * tempImage[i][j];
					}
				}
		...
		tempImage[i + dlg.m_UpSampleRate][j + dlg.m_UpSampleRate] =
			box[m][n] * m_InputImage[i * m_width + j];
...
void CImageProcessing1Doc::OnHistoEqual()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, value;
...
void CImageProcessing1Doc::OnTranslation()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j;
...
void CImageProcessing1Doc::OnMirrorHor()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j;
...
void CImageProcessing1Doc::OnMirrorVer()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j;
...
void CImageProcessing1Doc::OnRotation()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
...
void CImageProcessing1Doc::OnFrameSum()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CFile File;
	CFileDialog OpenDlg(TRUE);
...
void CImageProcessing1Doc::OnFrameSub()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CFile File;
	CFileDialog OpenDlg(TRUE);
...
void CImageProcessing1Doc::OnFrameMul()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CFile File;
	CFileDialog OpenDlg(TRUE);
...
void CImageProcessing1Doc::OnFrameDiv()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CFile File;
	CFileDialog OpenDlg(TRUE);
...
void CImageProcessing1Doc::OnFrameAnd()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CFile File;
	CFileDialog OpenDlg(TRUE);
...
void CImageProcessing1Doc::OnFrameOr()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CFile File;
	CFileDialog OpenDlg(TRUE);
...
void CImageProcessing1Doc::OnFrameComb()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CFile File;
	CFileDialog OpenDlg(TRUE);
...
void CImageProcessing1Doc::OnBinaryErosion()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, n, m;
...
void CImageProcessing1Doc::OnBinaryDilation()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, n, m;
...
void CImageProcessing1Doc::OnGrayErosion()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, n, m, h;
...
void CImageProcessing1Doc::OnGrayDilation()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, n, m, h;
...
void CImageProcessing1Doc::OnLowPassFilter()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j;
...
void CImageProcessing1Doc::OnHighPassFilter()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j;
...
void CImageProcessing1Doc::OnMeanFilter()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, n, m;
...
void CImageProcessing1Doc::OnMedianFilter()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, n, m, index = 0;
...
void CImageProcessing1Doc::OnMaxFilter()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, n, m, index = 0;
	double** tempInputImage, Mask[9];
...
void CImageProcessing1Doc::OnMinFilter()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, n, m, index = 0;
	double** tempInputImage, Mask[9];
...
void CImageProcessing1Doc::OnFft2d()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
...
void CImageProcessing1Doc::OnFft1d(Complex* X, int N, int Log2N)
{
	// TODO: 여기에 구현 코드 추가.
	// 1차원 fft를 위한 함수
...
void CImageProcessing1Doc::OnShuffle(Complex* X, int N, int Log2N)
{
	// TODO: 여기에 구현 코드 추가.
...
void CImageProcessing1Doc::OnButterfly(Complex* X, int N, int Log2N, int mode)
{
	// TODO: 여기에 구현 코드 추가.
...
int CImageProcessing1Doc::OnReverseBitOrder(int index, int Log2N)
{
	// TODO: 여기에 구현 코드 추가.
	int i, X, Y;
...
void CImageProcessing1Doc::OnIfft2d()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, Num, Log2N;
	Complex* Data;
...
void CImageProcessing1Doc::OnIfft1d(Complex* X, int N, int Log2N)
{
	// TODO: 여기에 구현 코드 추가.
	OnShuffle(X, N, Log2N);
...
void CImageProcessing1Doc::OnLpfFrequency()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, x, y;
	double temp, D, N;
...
void CImageProcessing1Doc::OnHpfFrequency()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	int i, j, x, y;
	double temp, D, N;
